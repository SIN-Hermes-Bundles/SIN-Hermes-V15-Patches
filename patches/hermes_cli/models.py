...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "custom":
        ...
        if base_url:
            ...
            if live:
                return live
    if normalized.startswith("custom:") and normalized != "custom":
        # Named custom providers (e.g. custom:fireworks) — load config,
        # resolve the entry, probe /v1/models live so the picker shows
        # whatever models the endpoint actually serves.
        try:
            from hermes_cli.config import load_config
            cfg = load_config()
