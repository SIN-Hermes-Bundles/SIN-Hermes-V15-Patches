...
# Register tool_search meta-tools (always available)
try:
    from tools.tool_search import register_tool_search
    register_tool_search()
except Exception as e:
    logger.debug("tool_search registration failed: %s", e)
...
# Full session tool names *before* tool_search deferral.
_all_session_tool_names: List[str] = []

# Compact catalog for tool_search when deferred mode is active.
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        if cached is not None:
            # Update global tool-name trackers so downstream callers see
            # consistent state even on a cache hit.
            global _last_resolved_tool_names, _all_session_tool_names, _deferred_catalog
            _last_resolved_tool_names = [t["function"]["name"] for t in cached]
            _all_session_tool_names = list(_last_resolved_tool_names)
            _deferred_catalog = [
                {**d["function"], "toolset": registry.get_tool_to_toolset_map().get(d["function"]["name"], "")}
                for d in cached
            ] if cached else []
...
def handle_function_call(
    ...
    try:
        ...
        if function_name in ("tool_search", "tool_details"):
            try:
                from tools.tool_search import search_tools, get_tool_details as _get_tool_details
                if function_name == "tool_search":
                    result = search_tools(function_args)
                else:
                    result = _get_tool_details(function_args)
            except Exception as e:
        ...
        if function_name == "execute_code":
            ...
            sandbox_enabled = enabled_tools if enabled_tools is not None else (_all_session_tool_names or _last_resolved_tool_names)
