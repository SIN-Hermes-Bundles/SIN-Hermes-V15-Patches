    ...
    if (
        ...
        if agent.context_compressor.should_compress(_preflight_tokens):
            ...
            for _pass in range(3):
                ...
                # Evict loaded tool schemas before compression to free context
                if getattr(agent, "_tool_search_active", False):
                    _evict_pinned = getattr(agent, "_pinned_tool_names", set()) | {"tool_search", "tool_details"}
                    agent.tools = [t for t in agent.tools if t["function"]["name"] in _evict_pinned]
                    agent._loaded_tools = set(_evict_pinned)
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                # Auto-load tool schemas requested via tool_details
                # (tool_search activation).  The model discovers a tool,
                # then needs its schema appended to agent.tools so the
                # next API call includes it.
                _ts_active = getattr(agent, "_tool_search_active", False)
                if _ts_active and assistant_message.tool_calls:
                    try:
                        import json
                        from model_tools import registry as _mtr
                        for _tc in assistant_message.tool_calls:
                            if _tc.function.name == "tool_details":
                                _td_args = json.loads(_tc.function.arguments)
                                _td_name = _td_args.get("name", "")
                                if _td_name and _td_name not in agent._loaded_tools:
                                    _td_schema = _mtr.get_single_definition(_td_name)
                                    if _td_schema:
                                        agent.tools.append(_td_schema)
                                        agent._loaded_tools.add(_td_name)
                                        agent.valid_tool_names.add(_td_name)
                    except Exception:
                        pass
