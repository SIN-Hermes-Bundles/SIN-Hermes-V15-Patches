...
from hermes_cli.config import load_config as _load_agent_config_init
from hermes_cli.timeouts import get_provider_request_timeout
from hermes_constants import get_hermes_home
from model_tools import check_toolset_requirements, get_tool_definitions
from model_tools import should_defer_tools, _estimate_tool_tokens, _all_session_tool_names
...
def init_agent(
    ...
    # Deferred tool loading (tool_search) — reduce initial tool schema size.
    agent._tool_search_active = False
    agent._loaded_tools: set = set()
    agent._pinned_tool_names: set = set()

    if agent.tools:
        try:
            _tool_search_cfg = _load_agent_config_init().get("tool_search", {})
        except Exception:
            _tool_search_cfg = {}
        _mode = str(_tool_search_cfg.get("mode", "auto")).lower()
        if _mode != "never":
            _token_est = _estimate_tool_tokens(agent.tools)
            _ctx = _load_agent_config_init().get("context_length", 128000)
            _threshold = float(_tool_search_cfg.get("threshold", 0.10))
            _pinned = set(_tool_search_cfg.get("pinned_tools", []))

            if should_defer_tools(_token_est, _ctx, mode=_mode, threshold=_threshold):
                agent._tool_search_active = True
                agent._pinned_tool_names = _pinned
                agent._loaded_tools = _pinned | {"tool_search", "tool_details"}
                agent.valid_tool_names = agent._loaded_tools.copy()
                agent.tools = [
                    t for t in agent.tools
                    if t["function"]["name"] in agent._loaded_tools
                ]
                if not agent.quiet_mode:
                    _full_count = len(_all_session_tool_names) if _all_session_tool_names else 0
                    print(f"🔎 Tool search active: {len(agent.tools)} pinned + {_full_count} deferred tools available")
