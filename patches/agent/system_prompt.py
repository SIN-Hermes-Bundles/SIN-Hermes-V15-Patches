...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Tool-search catalog: when deferred tool loading is active, inject the
    # compact catalog so the model knows what tools exist beyond the pinned set.
    if getattr(agent, "_tool_search_active", False):
        ...
        _catalog_prompt = build_tool_catalog_prompt(_deferred_catalog or [])
        if _catalog_prompt:
            stable_parts.append(_catalog_prompt)
